...
<body class="bg-white dark:bg-slate-900 flex flex-col min-h-screen font-sans text-gray-900 dark:text-gray-200 antialiased">
                ...
                <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                    ...
                    <div class="flex items-center md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse relative">
                        @auth
                            <button type="button" class="flex text-sm bg-gray-800 rounded-full md:me-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" id="user-menu-button" aria-expanded="false">
                                ...
                                <img id="dropdown-avatar" class="w-8 h-8 rounded-full" src="{{ Auth::user()->avatar ? asset('storage/avatars/' . Auth::user()->avatar) : asset('images/default_avatar.png') }}" alt="Foto pengguna">
                            </button>
                    ...
                    <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-user-default">
                        <ul class="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 dark:border-slate-700 rounded-lg bg-gray-50 dark:bg-slate-800 md:space-x-8 md:flex-row md:mt-0 md:border-0 md:bg-white dark:md:bg-slate-800">
                            <li><a href="{{ route('Beranda') }}" class="nav-link-default {{ request()->routeIs('Beranda') ? 'active' : '' }}">Home</a></li>
                            <li><a href="{{ route('about') }}" class="nav-link-default {{ request()->routeIs('about') ? 'active' : '' }}">About</a></li>
                            <li><a href="{{ route('services') }}" class="nav-link-default {{ request()->routeIs('services') ? 'active' : '' }}">Services</a></li>
                            <li><a href="{{ route('gallery') }}" class="nav-link-default {{ request()->routeIs('gallery') ? 'active' : '' }}">Gallery</a></li>
                            <li><a href="{{ route('contact') }}" class="nav-link-default {{ request()->routeIs('contact') ? 'active' : '' }}">Contact</a></li>
                        </ul>
                    </div>
                </div>
            </nav>
    ...
    @if (session('success'))
        <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-4 mx-auto max-w-4xl" role="alert">
            {{ session('success') }}
        </div>
    @endif
    @if (session('error'))
        <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4 mx-auto max-w-4xl" role="alert">
            {{ session('error') }}
        </div>
    @endif
    @if ($errors->any() && !request()->routeIs('registerevent'))
        <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4 mx-auto max-w-4xl" role="alert">
            <ul>
    ...
    <footer class="bg-slate-800 dark:bg-slate-900 text-gray-300 dark:text-gray-400 text-center p-8 mt-auto">
        <p>© {{ date('Y') }} Festivalan. Hak Cipta Dilindungi.</p>
    </footer>
        ...
        <div id="eventRegistrationModalContent" class="relative mx-auto p-6 sm:p-8 w-full max-w-lg shadow-2xl rounded-xl bg-white dark:bg-slate-800 text-gray-900 dark:text-white animate-slide-up" style="animation-duration: 0.3s;">
            ...
            <form id="eventRegistrationForm" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="modalUserName" name="name" required class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent sm:text-sm" placeholder="John Doe" value="{{ Auth::check() ? Auth::user()->name : '' }}">
                </div>
                <div>
                    <label for="modalUserEmail" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Alamat Email</label>
                    <input type="email" id="modalUserEmail" name="email" required class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent sm:text-sm" placeholder="anda@example.com" value="{{ Auth::check() ? Auth::user()->email : '' }}">
                </div>
    ...
    <script>
        ...
        if (hamburgerMenuButton) {
            const targetNavId = hamburgerMenuButton.getAttribute('aria-controls');
            const targetNav = document.getElementById(targetNavId);
            if (targetNav) {
                hamburgerMenuButton.addEventListener('click', function() {
                    targetNav.classList.toggle('hidden');
                    targetNav.classList.toggle('md:flex');
                });
            }
        }

        window.openEventRegistrationModal = function(eventName, eventId) {
